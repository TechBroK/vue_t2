  ...
  <main class="auth-page">
    <form class="auth-card" @submit.prevent="handleSubmit" novalidate>
      ...
      <div class="auth-actions">
        ...
        <router-link class="btn ghost" :to="{ name: 'Auth', query: { page: 'login' } }">Login</router-link>
      </div>
    </form>
  </main>
</template>
...
onMounted(() => {
  if (route.query && route.query.from) {
    notify.show('Please sign up or login to continue', { type: 'info' })
  }
})

function validate() {
  const e = {}
  if (!name.value) e.name = 'Full name is required'
  ...
  if (password.value && confirmPassword.value && password.value !== confirmPassword.value) e.confirmPassword = 'Passwords do not match'
  return e
}
...
async function handleSubmit() {
  ...
  try {
  const res = await Promise.resolve(auth.signup({ name: name.value, email: email.value, password: password.value }))
